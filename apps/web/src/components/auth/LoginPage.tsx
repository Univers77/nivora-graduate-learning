  ...
  return <main className="grid-noise min-h-dvh overflow-hidden bg-ink px-5 py-8">
    <section className="mx-auto grid min-h-[calc(100dvh-4rem)] max-w-6xl items-center gap-8 lg:grid-cols-[1.05fr_.95fr]">
      <div className="relative">
        ...
        <h1 className="mt-4 max-w-3xl text-5xl font-bold tracking-[-.07em] sm:text-7xl">Aprendizaje ejecutivo, convertido en dominio real.</h1>
        <p className="mt-5 max-w-xl text-lg text-white/55">Una plataforma académica con ruta guiada, práctica activa y memoria de aprendizaje para avanzar con precisión desde la primera sesión.</p>
        <div className="mt-8 grid gap-3 sm:grid-cols-3">{["Inicio desde cero","Guía contextual","Memoria trazable"].map((item)=><div className="motion-card rounded-2xl border border-white/10 bg-white/[.035] p-4" key={item}><ShieldCheck className="text-lime" size={18}/><p className="mt-4 text-sm font-semibold">{item}</p></div>)}</div>
      </div>
      <div className="relative">
