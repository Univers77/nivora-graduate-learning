...
export function FloatingGuide() {
  ...
  const writeObsidianNote = useCallback(async (root: DirectoryHandle, markdown: string) => {
    const oscar = await root.getDirectoryHandle("Oscar Vargas", { create: true });
    const nivora = await oscar.getDirectoryHandle("NIVORA", { create: true });
  ...
  if (!session.guideOpen) {
    return <button onClick={()=>setGuideOpen(true)} className="fixed bottom-5 right-5 z-50 inline-flex items-center gap-2 rounded-full bg-lime px-4 py-3 text-sm font-bold text-ink shadow-2xl shadow-black/40"><Compass size={18}/>Guía</button>;
  }
  ...
  return <aside className="fixed bottom-4 right-4 z-50 w-[min(390px,calc(100vw-2rem))] overflow-hidden rounded-[28px] border border-lime/25 bg-[#120b0f]/95 shadow-2xl shadow-black/50 backdrop-blur">
    <div className="border-b border-white/10 bg-gradient-to-br from-lime/15 to-cyan/10 p-5"><div className="flex items-start justify-between gap-4"><div><p className="eyebrow">Guía de Oscar</p><h2 className="mt-2 text-2xl font-bold">Paso a paso</h2><p className="mt-2 text-sm text-white/55">Objetivo: máxima retención, progreso desde cero y memoria trazable.</p></div><button aria-label="Cerrar guía" onClick={()=>setGuideOpen(false)} className="rounded-full border border-white/10 p-2 text-white/55"><X size={17}/></button></div></div>
    <div className="max-h-[62vh] space-y-3 overflow-y-auto p-4">{steps.map(([n,title,text,to])=><Link to={to} onClick={()=>completeStep(title)} className="block rounded-2xl border border-white/10 bg-white/[.035] p-4 transition-colors hover:border-lime/35" key={n}><div className="flex gap-3"><span className="font-mono text-xs text-lime">{n}</span><div><p className="font-semibold">{title}</p><p className="mt-1 text-sm text-white/45">{text}</p></div></div></Link>)}</div>
