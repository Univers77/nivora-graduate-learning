import { BookOpen, CheckCircle2, DatabaseZap, GraduationCap, LogOut, RotateCcw, Sparkles, X } from "lucide-react";
...
export function FloatingGuide() {
  ...
  return <aside className="guide-sheet fixed bottom-[calc(.75rem+env(safe-area-inset-bottom))] right-3 z-50 flex max-h-[calc(100dvh-1.5rem-env(safe-area-inset-top)-env(safe-area-inset-bottom))] w-[min(370px,calc(100vw-1.5rem))] flex-col overflow-hidden rounded-[28px] border border-lime/25 bg-[#120b0f]/95 shadow-2xl shadow-black/50 backdrop-blur">
    ...
    <div className="grid gap-2 border-t border-white/10 p-4 sm:grid-cols-2"><div className="rounded-2xl border border-lime/20 bg-lime/10 p-4 sm:col-span-2"><div className="flex items-center gap-2 text-sm font-bold text-lime"><DatabaseZap size={16}/>{t.guide.autoSave}</div><p className="mt-2 text-xs text-white/45">{t.guide.autoSaveText}</p></div><button onClick={reset} className="inline-flex items-center justify-center gap-2 rounded-full border border-cyan/25 px-4 py-3 text-sm text-cyan"><RotateCcw size={16}/>{t.guide.reset}</button><button onClick={logout} className="inline-flex items-center justify-center gap-2 rounded-full border border-white/10 px-4 py-3 text-sm text-white/70"><LogOut size={16}/>{t.guide.logout}</button></div>
    <div className="flex flex-wrap items-center gap-2 px-5 pb-4 text-xs text-white/40"><Sparkles size={14} className="text-lime"/><span>{events.length ? `${events.length} ${t.guide.evidence}` : t.guide.noEvidence}</span><CheckCircle2 size={14} className="text-lime"/><span>{t.guide.ledger}</span><BookOpen size={14} className="text-cyan"/></div>
  </aside>;
}
